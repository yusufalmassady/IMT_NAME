...
void MI2C_voidSendSlaveAddressWithWriteReq(unsigned char Copy_u8SlaveAddress)
{
...
void MI2C_voidSendSlaveAddressWithReadReq(unsigned char Copy_u8SlaveAddress)
{
	TWDR = (Copy_u8SlaveAddress << 1);                     // Address
...
void MI2C_voidWriteByte(unsigned char Copy_u8Byte)
{
...
unsigned char MI2C_u8ReadByte(void)
{
	TWCR = (1<< TWINT) | (1<< TWEN);             // Clear TWI interrupt flag,Enable TWI
	while(!(TWCR & (1<< TWINT)));                // Wait till complete TWDR byte transmitted
...
unsigned char MI2C_voidSlaveReadByte(void)
{
...
void MI2C_voidSlaveWriteByte(unsigned char Copy_u8Byte)
{
	TWDR = Copy_u8Byte;
	TWCR = (1<<TWINT) | (1<<TWEA) | (1<<TWEN);        // Clear TWI interrupt flag,Get enable acknowledgment, Enable TWI
	while(!(TWCR & (1<<TWINT)));                      // Wait for TWINT flag
